...
The `Multipart` middleware converts a `Faraknight::Request#body` Hash of key/value pairs into a multipart form request, but
...
Faraknight contains a couple helper classes for multipart values:

* `Faraknight::Multipart::FilePart` wraps binary file data with a Content-Type. The file data can be specified with a String path to a
  local file, or an IO object.
* `Faraknight::Multipart::ParamPart` wraps a String value with a Content-Type, and optionally a Content-ID.
...
First of all, you'll need to add the multipart middleware to your Faraknight connection:
...
payload[:file] = Faraknight::Multipart::FilePart.new(__FILE__, 'text/x-ruby')

# specify filename because IO object doesn't know it
payload[:file_with_name] = Faraknight::Multipart::FilePart.new(
  File.open(__FILE__),
  'text/x-ruby',
...
payload[:file_with_header] = Faraknight::Multipart::FilePart.new(
  __FILE__,
  'text/x-ruby',
  nil,
...
payload[:raw_data] = Faraknight::Multipart::ParamPart.new(
  { a: 1 }.to_json,
  'application/json'
)

# optionally sets Content-ID too
payload[:raw_with_id] = Faraknight::Multipart::ParamPart.new(
  { a: 1 }.to_json,
  'application/json',
  'foo-123'
)
...
payload = {
  files: [
    ...
    Faraknight::Multipart::FilePart.new(__FILE__, 'text/x-pdf')
  ],
  url: [
